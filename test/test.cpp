...
TEST_CASE("Testing Levels"){
    ...
    SECTION("Checking Description loading"){
        ...
        REQUIRE(test.get_next()["<Move>"]["forward"] != nullptr);
        REQUIRE(test.get_next()["<Move>"]["back"] != nullptr);
        REQUIRE(test.get_next()["<Move>"]["left"] != nullptr);
        REQUIRE(test.get_next()["<Move>"]["right"] != nullptr);

        REQUIRE(test1.get_next()["<Move>"]["back"] != nullptr);
    }
